...
const routes = [
  {
    ...
    steps: ['🚶 Walk 5 min to Guindy Metro', '🚇 Metro: Guindy → Central (18 min)', '🚌 Bus 21C → Destination (9 min)'],
  ...
  },
  {
    ...
    steps: ['🚴 Cycle 8 min to Metro', '🚇 Metro: Anna Nagar → Egmore (22 min)', '🚶 Walk 10 min'],
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#020a06', paddingTop: '100px' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '40px 24px', position: 'relative', zIndex: 1 }}>
        ...
        <h1 style={{
          fontSize: 'clamp(36px, 6vw, 60px)', fontWeight: '900', marginBottom: '16px',
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '16px', lineHeight: '1.7', marginBottom: '48px' }}>
          AI-powered multi-modal journey planner across Metro, Bus, Train & Auto.
        </p>

        {/* Search */}
        <div style={{
          background: 'rgba(74,222,128,0.04)', border: '1px solid rgba(74,222,128,0.2)',
          borderRadius: '28px', padding: '32px', marginBottom: '36px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input value={from} onChange={e => setFrom(e.target.value)} placeholder="From: Enter origin"
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input value={to} onChange={e => setTo(e.target.value)} placeholder="To: Enter destination"
                style={{ width: '100%', padding: '16px 16px 16px 40px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(74,222,128,0.15)', borderRadius: '14px', color: '#fff', fontSize: '15px', outline: 'none' }} />
            </div>
            <button onClick={() => { if (from && to) setSearched(true); }} style={{
              padding: '16px', background: 'linear-gradient(135deg, #4ade80, #16a34a)',
              border: 'none', borderRadius: '14px', color: '#020a06',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {routes.map((route, i) => (
                <div key={i} onClick={() => setSelected(i)} style={{
                  padding: '24px', background: selected === i ? route.bg : 'rgba(255,255,255,0.02)',
                  border: `1px solid ${selected === i ? route.color + '50' : 'rgba(255,255,255,0.06)'}`,
                  borderRadius: '20px', cursor: 'pointer', transition: 'all 0.25s ease',
                  boxShadow: selected === i ? `0 8px 32px ${route.color}15` : 'none',
                }}>
