  ...
  return (
    <div style={{ minHeight: '100vh', background: '#020a06', paddingTop: '100px' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '40px 24px', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '16px', lineHeight: '1.7', marginBottom: '48px' }}>Track CO₂ savings and earn GreenCoins redeemable for transit rewards.</p>

        {/* Score Card */}
        <div style={{
          background: 'linear-gradient(135deg, rgba(74,222,128,0.08), rgba(134,239,172,0.05))',
          border: '1px solid rgba(74,222,128,0.2)', borderRadius: '28px',
          ...
          {[
            ...
            { val: '🥈', label: 'Silver Commuter Badge', color: '#fff' },
          ].map((item, i) => (
            <div key={i}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '32px' }}>
          {trips.map((trip, i) => (
            <div key={i} style={{
              ...
              border: '1px solid rgba(74,222,128,0.08)', borderRadius: '14px',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
          {rewards.map((r, i) => (
            <div key={i} style={{
              ...
              background: redeemed === i ? 'rgba(74,222,128,0.1)' : 'rgba(255,255,255,0.02)',
              border: `1px solid ${redeemed === i ? '#4ade80' : 'rgba(74,222,128,0.1)'}`,
              ...
              <button onClick={() => setRedeemed(i)} disabled={totalPoints < r.points} style={{
                width: '100%', padding: '9px',
                background: redeemed === i ? 'linear-gradient(135deg, #4ade80, #16a34a)' : totalPoints >= r.points ? 'rgba(74,222,128,0.12)' : 'rgba(255,255,255,0.04)',
                border: `1px solid ${totalPoints >= r.points ? '#4ade80' : 'rgba(255,255,255,0.06)'}`,
                borderRadius: '10px', color: totalPoints >= r.points ? (redeemed === i ? '#020a06' : '#4ade80') : 'rgba(255,255,255,0.2)',
                cursor: totalPoints >= r.points ? 'pointer' : 'not-allowed',
                fontSize: '13px', fontWeight: '700',
              }}>{redeemed === i ? '✅ Redeemed!' : 'Redeem'}</button>
            </div>
          ))}
        </div>
      </div>
